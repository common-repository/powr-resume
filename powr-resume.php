<?php
    ...
    Plugin URI: https://www.powr.io/plugins/resume
    ...
    */
    ...
    class Powr_Resume extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Resume.
        </p>
        <p>
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_resume_menu' );
    function powr_resume_menu() {
      add_menu_page( 'POWr Resume', 'POWr Resume', 'manage_options', 'powr-resume-settings', 'powr_resume_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_resume_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/resume';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/resume';
      }
    ...
    register_activation_hook(__FILE__, 'powr_resume_plugin_activate');
      add_action('admin_init', 'powr_resume_plugin_redirect');

      function powr_resume_plugin_activate() {
      add_option('powr_resume_plugin_do_activation_redirect', true);
      }
      ...
      function powr_resume_plugin_redirect() {
      if (get_option('powr_resume_plugin_do_activation_redirect', false)) {
          delete_option('powr_resume_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-resume-settings&' );
          }
       }
      }
